...
*/
void QCPAxis::scaleRange(double factor, double center, float xLowerBoundary, float xUpperBoundary)
{
    QCPRange oldRange = mRange;
  if (mScaleType == stLinear)
  {
    ...
    if(newRange.lower < xLowerBoundary) newRange.lower = xLowerBoundary;
    newRange.upper = (mRange.upper-center)*factor + center;
    if(newRange.upper > xUpperBoundary) newRange.upper = xUpperBoundary;
    ...
    if (mRangeZoom != 0)
    {
      ...
      if (mRangeZoom.testFlag(Qt::Horizontal))
      {
        ...
        if (mRangeZoomHorzAxis.data())
          mRangeZoomHorzAxis.data()->scaleRange(factor, mRangeZoomHorzAxis.data()->pixelToCoord(event->pos().x()), xLowerBoundary, xUpperBoundary); // Lenny
      }
